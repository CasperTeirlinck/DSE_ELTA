...
class CurrentVariables:
    def __init__(self, conceptnumber=2, n_engines=1, wing_mounted=False, T_tail=False, x_cg_pass=0.97, x_cg_batt=1.96,
                 ...
                 spinner_length=0.365, bulkhead_loc=0.875, cabin_length=1.3, tailheight=1.2, n_blades=2, batterypilot=True):

        self.batterypilot=batterypilot

        ## Requirements
        self.sto         = 500                                          # [m] take-off distance
        self.range_WPL   = 100*9.80665                                  # [N] Payload weight for endurance
        self.WPL         = 200*9.80665                                  # [N] Payload weight, full payload (endurance)
        ...
        self._CD0to       = 0.0380                                       # drag constant
        self._CD0clean    = 0.0250                                       # drag constant
        ...
        self.A_h         = 2.7                                          # [-] (3-5)aspect ratio of the horizontal tailplane
        self.A_v         = 1.49                                         # [-] (1-2) aspect ratio of the vertical tailplane
        self.x_htail     = 6.9                                          # [m] location of the ac of the horizontal tail
        self.x_vtail     = 6.9                                          # [m] location of the ac of the vertical tail
        ...
        self.A           = 10.1                                         # aspect ratio of the main wing
        ...
        self.V_eas       = 105*0.514444444                               # [m/s] cruise velocity EAS
        ...
        # Miscellaneous
        self._eff_propeller = 0.88
        self.T_propeller = None
        self.tail_ready  = False
        self.tcwing = 0.12

        ## Calculated values
        self.CLto        = np.array(self.CLmaxto / (1.1 ** 2))          # take-off CL, = CLmax,TO/1.1^2
        ...
        self.bmf_r       = None                                         # [-] Battery mass fraction for range (half PL)
        self.bmf_e       = None                                         # [-] Battery mass fraction for endurance
    ...
    @property
    def eff_propeller(self):
        return self._eff_propeller

    @eff_propeller.setter
    def eff_propeller(self, val):
        self._eff_propeller = val
        self.eff_tot_prop = 0.95 * self.eff_propeller  # Total propulsion efficiency (motor and bat)

    @property
    def CLmaxto(self):
        return self._CLmaxto

    @CLmaxto.setter
    def CLmaxto(self, val):
        self._CLmaxto = val
        self.CLto = np.array(self.CLmaxto / (1.1 ** 2))  # take-off CL, = CLmax,TO/1.1^2
        self.CDto = self.CD0to + self.CLto ** 2 / (pi * self.A * self.e)  # drag during takeoff
        self.CLclimb     = self.CLmaxto - 0.2                           # CLmax - safety margin fo crimb gradient
    ...
    def init_single_engine(self):
        ...
        self._CLmaxto = 2.0                                              # CLmax take-off
        # self.CLmaxland = np.array([1.7, 2.0, 2.3])                      # CLmax landing
        self.CLmaxland = 1.7                    # CLmax landing
    ...
    def init_multi_engine(self):
        ...
        self._CLmaxto     = 1.9                                          # CLmax take-off
