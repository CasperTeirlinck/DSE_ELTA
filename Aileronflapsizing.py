...
P           = 20*np.pi/180           #[rad/s] roll rate requirement
...
deltaC_l_max= 1.13 #[-] change in C_l due to flaps at max deflection
...
def Flapsurface(c_l_a,AR,a_max,C_L_max_req,S,deltaC_l_max):
...
def Flappos(b,S,c_root,taperrat,b2,delta_a,c_l_delta_a,V,P,c_tip,c_l_a,AR,a_max,C_L_max_req,deltaC_l_max):
    f2 = aileronstartinner(b,S,c_root,taperrat,b2,delta_a,c_l_delta_a,V,P) - 1  #outer location of flap

    A = (c_root-c_tip)/b
    B = -c_root
    C = Flapsurface(c_l_a,AR,a_max,C_L_max_req,S,deltaC_l_max) - c_root*f2 + A*f2**2
