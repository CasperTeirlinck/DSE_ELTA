...
def num_of_cells(variables, E_req, eta_batt_load = 1.0):
    ...
    # Required capacity
    C_req = E_req/(variables.V_req_batt * eta_batt_load * (variables.DoD/100))
    #C_req = 233

    # Number of cells is series for voltage
    Ns = variables.V_req_batt/variables.batt_cell_V_nom
    # Rounding to the smallest integer larger than Ns
    Ns = ceil(Ns)

    # Number of cells in parallel for capacity
    Np = C_req/variables.batt_cell_C_Ah
    Np = ceil(Np)

    # Check if enough current is generated
    if Np*variables.batt_cell_I_max < variables.I_req_batt:
